...
class VistapoolDataUpdateCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    def apply_optimistic(self, value_path: str, value: Any) -> None:
        """Reflect a just-written value and protect it from stale pushes."""
        # Stale Firestore snapshots in flight when we wrote can briefly
        # carry the pre-write state and would clobber the optimistic value;
        # record the pending write so _apply_remote_data can hold the line
        # until the controller confirms or OPTIMISTIC_TTL_SECONDS elapses.
